...
<body>
	...
	<div class="container">
		...
		<?php } elseif (isset($_POST["surl"]) && isset($_POST["pwd"])) {
			CheckPassword();
			$surl = $_POST["surl"];
			$pwd = $_POST["pwd"];
			...
			if ($json2 != 1) {
				...
				if ($filejson["errno"] == -21) {
					...
					for ($i = 0; $i < count($filejson["list"]); $i++) {
						...
						if ($file["isdir"] == 0) {
							$filecontent .= '<li class="list-group-item border-muted rounded text-muted py-2"><i class="far fa-file mr-2"></i>
						...
						<span class="float-right">' . formatSize($file["size"]) . '</span></li>';
							...
							$filecontent .= '<li class="list-group-item border-muted rounded text-muted py-2"><i class="far fa-folder mr-2"></i><a href="javascript:ToSharePage(\'' . $surl . '\');">' . $file["server_filename"] . '</a><span class="float-right"></span></li>';
						}
					}
				...
				if (isset($_POST["fs_id"]) && isset($_POST["time"]) && isset($_POST["sign"]) && isset($_POST["randsk"]) && isset($_POST["share_id"]) && isset($_POST["uk"])) {
					...
					if ($json4["errno"] == 0) {
						...
						if ($realLink == "") {
							...
							<p class="card-text">文件MD5：<b>' . $md5 . '</b></p>
							<p class="card-text">上传时间：<b>' . date("Y年m月d日 H:i:s", $server_ctime) . '</b></p>
							<p class="card-text"><a href="http://' . $realLink . '" target="_blank">下载链接（不安全）</a>
							<a href="https://' . $realLink . '" target="_blank">下载链接（安全）</a></p>
